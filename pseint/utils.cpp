...
void ExeError_impl(int num, std::string s) { 
	if (Inter.EvaluatingForDebug()) {
		...
		for(int i=Inter.GetBacktraceLevel()-1;i>0;i--) {  
			...
			std::cout << "...dentro del subproceso " << fi.func_name << ", invocado desde la l�nea " << fi.loc.linea << "." << std::endl;
		}
		if (ExeInfoOn) {
...
void fixwincharset(std::string &s, bool reverse) {
	...
	if (reverse) {
		for(unsigned int i=0;i<s.size();i++) { 
			char &c=s[i];
			if (c==-96) c='\xE1'; // á
			else if (c==-126) c='\xE9'; // é
			else if (c==-95) c='\xED'; // í
			else if (c==-94) c='\xF3'; // ó
			else if (c==-93) c='\xFA'; // ú
			else if (c==-75) c='\xC1'; // Á
			else if (c==-112) c='\xC9'; // É
			else if (c==-42) c='\xCD'; // Í
			else if (c==-32) c='\xD3'; // Ó
			else if (c==-23) c='\xDA'; // Ú
			else if (c==-92) c='\xF1'; // ñ
			else if (c==-91) c='\xD1'; // Ñ
			else if (c==-83) c='\xA1'; // ¡
			else if (c==-88) c='\xBF'; // ¿
			else if (c==-127) c='\xFC'; // ü
			else if (c==-102) c='\xDC'; // Ü
		}
	} else {
		for(unsigned int i=0;i<s.size();i++) { 
			char &c=s[i];
			if (c=='\xE1') c=-96; // á
			else if (c=='\xE9') c=-126; // é
			else if (c=='\xED') c=-95; // í
			else if (c=='\xF3') c=-94; // ó
			else if (c=='\xFA') c=-93; // ú
			else if (c=='\xC1') c=-75; // Á
			else if (c=='\xC9') c=-112; // É
			else if (c=='\xCD') c=-42; // Í
			else if (c=='\xD3') c=-32; // Ó
			else if (c=='\xDA') c=-23; // Ú
			else if (c=='\xF1') c=-92; // ñ
			else if (c=='\xD1') c=-91; // Ñ
			else if (c=='\xA1') c=-83; // ¡
			else if (c=='\xBF') c=-88; // ¿
			else if (c=='\xFC') c=-127; // ü
			else if (c=='\xDC') c=-102; // Ü
		}
	}
}
