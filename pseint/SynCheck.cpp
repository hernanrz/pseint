...
static void FixAcentos(std::string &s) {
	for(char &c : s) {
		if (c=='\xF1' || c=='\xD1') c='N'; // ñ Ñ
		else if (c=='\xE1' || c=='\xC1') c='A'; // á Á
		else if (c=='\xE9' || c=='\xC9') c='E'; // é É
		else if (c=='\xED' || c=='\xCD') c='I'; // í Í
		else if (c=='\xF3' || c=='\xD3') c='O'; // ó Ó
		else if (c=='\xFA' || c=='\xDA') c='U'; // ú Ú
		else if (c=='\xFC' || c=='\xDC') c='U'; // ü Ü
	}
}
...
//						(cadena[i]>='a'&&cadena[i]<='z') || // no deberia hacer falta a esta altura
			...
			// En esta parte, según cada instrucción se verifican si los argumentos están bien. Los argumentos quedaron solos en cadena, la instrucción ya fue cortada.
			...
			if (inst.type==IT_INVOCAR) {
				...
				else { // entonces tiene argumentos, y requiere argumentos, ver que la cantidad está bien
