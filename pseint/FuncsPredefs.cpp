...
DataValue func_rc(ErrorHandler &err_handler, DataValue *arg) {
	...
	if (x<0) {
		err_handler.ErrorIfRunning(147,"Ra�z cuadrada de n�mero negativo.");
...
DataValue func_trunc(ErrorHandler &err_handler, DataValue *arg) {
	...
	int i = int(dbl);
	// intentar compensar algunos errores num�ricos... que al menos parezca el 
	// n�mero que se ve al escribir, el cual se redondea a 10 decimales... 
	// por eso uso un epsilon un poquito m�s chico que eso
