...
int main(int argc, char* argv[]) {
	...
	bool 
		...
		read_stdin=false; // leer el programa desde stdin en lugar de un archivo .psc
	int 
	...
	if (error || (fil_count==0 && !real_time_syntax && !read_stdin)) {
		...
		cout<<"      --stdin                lee el programa desde stdin en lugar de un archivo .psc"<<endl;
...
//			memoria->HardReset();
	...
	// Leer el archivo o stdin
	if (read_stdin) {
		if (user && check) show_user_info("Leyendo entrada y comprobando sintaxis...");
